...
class BuyoutContainer extends React.Component {
  constructor(props) {
    ...
    this.state = Object.assign(
      ...
      {
        customerCreate:
          this.props.byo && this.props.byo.customer ? false : true,
        mailBody: "",
        mailSubject: "",
        mailCC: "",
        mailAttachments: [],
  ...
  handleChangeCustomer(e) {
    ...
    if (fill.length > 0) {
      ...
      this.setState({
        customer: Object.assign({}, this.state.customer, { name }),
      });
    }
  }
  ...
  validateFields() {
    let errors = {};
    if (this.state.customer && this.state.customer.zip)
      errors.zip =
        this.state.customer.zip.search(/\d\d\d\d\d|\d\d\d\d\d-\d\d\d\d/) === -1
          ? true
          : false;
    if (this.state.customer && this.state.customer.phone)
      errors.phone =
        this.state.customer.phone.search(
          /\d\d\d(-|\.)\d\d\d(-|\.)\d\d\d\d|\(\d\d\d\)\s\d\d\d(-|\.)\d\d\d\d|\d\d\d\d\d\d\d\d\d\d/
        ) === -1
          ? true
          : false;
    if (this.state.customer && this.state.customer.email)
      errors.email =
        this.state.customer.email.search(/\w+@\w+\.\w+/) === -1 ? true : false;
    return errors;
  }
  ...
  generateErrors(errors) {
    ...
    return (e) => {
      ...
      this.props.throwAlert(
        "red",
        `Problem with the ${fields.join(", ")} field${
          fields.length > 1 ? "s" : ""
        }`
      );
    };
  ...
  handleSave(e) {
    ...
    if (!this.state.customer || !this.state.customer.name) {
      this.props.throwAlert(
        "red",
        "To save, you must fill in the customer name"
      );
    } else if (this.state.customerCreate) {
      this.props.saveByoThunk(
        this.props.token,
        [this.state],
        [this.state.customer, { id: "new" }]
      );
    } else {
      this.props.saveByoThunk(
        this.props.token,
  ...
  handleSubmit(e) {
    ...
    if (
      ...
      if (this.state.customerCreate) {
        this.props.saveByoThunk(
          this.props.token,
          [this.state, { status: "New", date: getDate() }],
          [this.state.customer, { id: "new" }]
        );
      } else {
        this.props.saveByoThunk(
  ...
  handleDelete(e) {
    e.preventDefault();
    if (confirm("Are you sure you wish to delete this buyout?"))
      this.props.deleteByoThunk(this.props.token, this.state.id, () => {
        this.props.history.push("/buyouts");
      });
  }
  ...
  handleNotify(e) {
    ...
    axios
      ...
      .then((res) => {
        ...
        return axios.post("/api/logs/new", {
          token: this.props.token,
          date: getDate(),
          activity: `<b>${this.props.user.fullName}<b> notified rep ${this.state.rep.fullName} that application ${this.state.action.appNumber} to ${this.state.action.leasingCompany} was ${this.state.action.status}`,
          action: this.state.action,
          byo: this.state.id,
          expiry: this.state.expiryTemp,
        });
      })
      .then((res) => {
        let overallStatus;
        if (this.state.action.status === "Working") overallStatus = "Working";
        else if (this.state.action.status === "Approved")
          overallStatus = "Approved";
        else if (this.state.action.status === "Declined")
          overallStatus = "Declined";

        if (overallStatus)
          return this.props.saveByoThunk(
            this.props.token,
            [
              this.state,
              { status: overallStatus, expiry: this.state.expiryTemp },
            ],
            [this.state.customer]
          );
        else
          return this.props.saveByoThunk(
            this.props.token,
            [this.state, { expiry: this.state.expiryTemp }],
            [this.state.customer]
          );
      })
      .then((res) => {
  ...
  handleNote(e) {
    ...
    actions[Number(e.target.id)].show = actions[Number(e.target.id)].show
      ? false
      : true;
  ...
  handleSaveAndNotify(e) {
    ...
    axios
      .put("/api/actions/", {
        token: this.props.token,
        action: Object.assign({}, this.state.action, { sentToRep: getDate() }),
      })
      .then((res) => {
        let soonToBeSubject = "";
        let soonToBeBody = "";
  ...
  handleWorkbookNotify(e) {
    ...
    Object.keys(this.state.checkedAttachments).forEach((index) => {
      if (this.state.checkedAttachments[index])
        mailAttachments.push(this.state.pdfs[index]);
    });
    this.setState({
      adminMode: "notifyByo",
      mailSubject: "",
      ...
      .then((res) => {
        ...
        return axios
          .post("/api/logs/new2", {
            token: this.props.token,
            date: getDate(),
            activity: `<b>${this.props.user.fullName}<b> notified rep ${this.state.rep.fullName}`,
            action: this.state.action,
            byo: this.state.id,
          })
          .then((res) => {
  ...
  handleCascade(e) {
    ...
    leases[this.state.calcTarget].workbook[name + "Upfront"] = round(
      product(
        e.target.value,
        leases[this.state.calcTarget].workbook.upfrontTax / 100
      )
    );
    leases[this.state.calcTarget].workbook[name + "Monthly"] = round(
      product(
        e.target.value,
        leases[this.state.calcTarget].workbook.monthlyTax / 100
      )
    );
    this.setState({ leases: leases });
  }
  ...
  handleTaxes(e) {
    ...
    let leases = Array.from(this.state.leases);
    leases[this.state.calcTarget].workbook[e.target.name] = e.target.value;
    leases[this.state.calcTarget].workbook[
      "currentEquipmentPayment" + type
    ] = round(
      product(
        leases[this.state.calcTarget].workbook.currentEquipmentPayment,
        e.target.value / 100
      )
    );
    leases[this.state.calcTarget].workbook[
      "currentServicePayment" + type
    ] = round(
      product(
        leases[this.state.calcTarget].workbook.currentServicePayment,
        e.target.value / 100
      )
    );
    leases[this.state.calcTarget].workbook["fuelFreight" + type] = round(
      product(
        leases[this.state.calcTarget].workbook.fuelFreight,
        e.target.value / 100
      )
    );
    leases[this.state.calcTarget].workbook["lateCharges" + type] = round(
      product(
        leases[this.state.calcTarget].workbook.lateCharges,
        e.target.value / 100
      )
    );
    leases[this.state.calcTarget].workbook["miscItems" + type] = round(
      product(
        leases[this.state.calcTarget].workbook.miscItems,
        e.target.value / 100
      )
    );
    this.setState({ leases: leases });
    ...
    axios
      ...
      .then(() => {
        return axios.post("/api/logs/new2", {
          ...
          }<b> generated PDFs for lease ${
            this.state.leases[this.state.calcTarget].number
          }`,
          byo: this.state.id,
        });
      })
  ...
  handleDeletePDF(e) {
    ...
    axios
      .delete(`/api/uploads/${e.target.id}?access_token=${this.props.token}`)
      .then((res) => {
  ...
  handleUploadPDF(e) {
    ...
    if (this.state.upload) {
      ...
      axios
        .post(
          `/api/uploads/buyout/${this.state.id}?access_token=${this.props.token}`,
          formData,
          { "content-type": "multipart/form-data" }
        )
        .then((res) => {
          if (res.data.color)
            this.props.throwAlert(res.data.color, res.data.message);
  ...
  handleRemoveAttachment(e) {
    ...
    let attachments = this.state.mailAttachments;
    attachments = [
      ...attachments.slice(0, Number(e.target.id)),
      ...attachments.slice(Number(e.target.id) + 1),
    ];
    this.setState({
      mailAttachments: attachments,
    });
    ...
    return (
      <div>
        <EditBuyout
          ...
          iAmAuthor={
            this.props.user
              ? this.props.user.email === this.state.rep.email
              : false
          }
          admin={this.props.user ? this.props.user.level === "Admin" : false}
          customers={
            this.props.customers
              ? this.props.customers.map((customer) => customer.name)
              : null
          }
          ...
          handleSubmit={
            disabled ? this.generateErrors(errors) : this.handleSubmit
          }
