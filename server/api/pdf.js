...
module.exports = require('express').Router()
  ...
  .post('/new', isLoggedIn, isAdmin, (req, res) => {
    ...
    .then(newUpload => {
      ...
      const fileName = `${values.leases[values.calcTarget].number}-${values.customer.name}-${(customer) ? 'Customer' : 'Rep'}-${getUglyDate()}.pdf`
      ...
      let header = () => {
        // logos
        doc.image(
          path.resolve(__dirname, '../../public/assets/img/myadmin_logo_FINAL.png'),
          docWidth - margin - 120, margin, {width: 120}
        )
